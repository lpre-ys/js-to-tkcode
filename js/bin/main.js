...
function initLog() {
  ...
  console.log(`${green}PROGRESS${reset} B: ${builded} W: ${writed} S: ${skipped} E: ${errorCount} total: ${total} / ${initTargetCount}`);
}
...
function build(scriptPath, lib) {
  return new Promise((resolve) => {
    ...
    makeDir(tkcodePath).then(() => {
      // ディレクトリの準備が終わったら、スクリプトとConfigを読み込む
      return Promise.all([readScript(scriptPath, lib), readAllLocalConst(scriptPath)]);
    }).then((results) => {
      // データがそろったらビルド
...
function makeDir(tkcodePath) {
  return new Promise((resolve) => {
    mkdirp(path.dirname(tkcodePath), (error) => {
...
function readAllLocalConst(scriptPath) {
  return Promise.all([readLocalConst(scriptPath), readLocalDirConst(scriptPath)])
    .then((results) => {
      const ret = {};
...
function readLocalDirConst(scriptPath) {
  ...
  return fsPromises.readFile(localDirConstPath, 'utf8').then((result) => {
...
function readLocalConst(scriptPath) {
  ...
  return fsPromises.readFile(localConstPath, 'utf8').then((result) => {
    return yaml.safeLoad(result);
  }).catch((error) => {
    // 読めなかった場合何もしない
    if (error.code !== 'ENOENT') {
